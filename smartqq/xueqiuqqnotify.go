...
func check_chatmd5(checkmd5 string, qq int) bool {
	checkmd5 = strings.TrimSpace(checkmd5)
	plain := Sprintf("xhbot%d", qq)
				...
				if msgtypeif, exist := msg["poll_type"]; exist {
					if msgtype, ok := msgtypeif.(string); ok {
						if msgtype == "message" || msgtype == "sess_message" {
							...
							if gauthchatqq.check(fromqq) == false {
								if strings.Contains(txt, "@"+gLoginName) {
									...
									if err != nil {
										...
										if check_chatmd5(wakeupmd5, fromqq) {
											gauthchatqq.add(fromqq)
							...
							aireply, err := getaireplay(fromqq, txt)
							...
							if err == nil {
								...
								if gauthchatqq.check(send_qq) == false {
									if strings.Contains(txt, "@"+gLoginName) {
										wakeupmd5, err := getReValue([]byte(txt), `\s*@xhbot\s*wakeup\s*(\w{6}})\s*`)
										if err != nil {
											...
											if check_chatmd5(wakeupmd5, send_qq) {
												gauthchatqq.add(send_qq)
...
var MSG_HEAD map[int]string = map[int]string{
	...
	MSG_EDIT:       "心大 编辑了:",
	MSG_DEL:        "心大 删除了:",
	MSG_STOCK_ADD:  "心大 增加自选:",
	MSG_STOCK_EDIT: "心大 编辑自选:",
	MSG_STOCK_DEL:  "心大 删除自选:",
}
...
func sendmailoreverote(msg notifymsg) error {
	...
	if len(msg.msg.title) > 0 {
		subject = Sprintf("(id=%d,rtid=%d)%s %s", msg.msg.id, msg.msg.retweet_status_id, MSG_HEAD[msg.itype], msg.msg.title)
	} else {
		subject = Sprintf("(id=%d,rtid=%d)%s %s", msg.msg.id, msg.msg.retweet_status_id, MSG_HEAD[msg.itype], msg.msg.description)
	}
	...
	for {
		...
		case msg := <-gxuueqiuusrsendqueue:
			...
			for _, v := range msg.groupuins {
				...
				if len(msg.msg.title) > 0 {
					sendgroupmsg(v.uin, Sprintf(`%s %s( http://xueqiu.com/%d/%d ,rtid=%d)`, MSG_HEAD[msg.itype], trimforqqmsg(erasebractket(msg.msg.title)), msg.uid, msg.msg.id, msg.msg.retweet_status_id), COLOR[color])
				} else {
					sendgroupmsg(v.uin, Sprintf(`%s %s( http://xueqiu.com/%d/%d ,rtid=%d)`, MSG_HEAD[msg.itype], trimforqqmsg(erasebractket(msg.msg.description)), msg.uid, msg.msg.id, msg.msg.retweet_status_id), COLOR[color])
				}
			}
